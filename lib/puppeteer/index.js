...
(async function launch(options) {
    ...
    const lotDetails = await page.evaluate(() => {
        const label = Array.from(document.getElementsByClassName('lot-details-information-label')).map(el => el.innerText);
        const value = Array.from(document.getElementsByClassName('lot-details-information-value')).map(el => el.innerText);
        return label.map((label, index) => ({ label, value: value[index] }));
    });
