...
async function humanClickHandle(page: any, el: any) {
...
async function listExportCandidates(page: any) {
...
async function findVisibleExportControl(page: any) {
...
async function waitForAnyNewFileStable(
	dir: string,
	startedAt: number,
...
export const logInToCopart = async function (landingUrl: string, targetUrl: string) {
	...
	try {
		...
		const memberSignin = await signin.$('[data-uname="homePageMemberSignIn"]');
		if (memberSignin) {
		...
		const submitSelector = isMemberLogin
			? '.cprt-btn-blue.p-full-width.sign-in-button'
			: '[data-uname="loginSigninmemberbutton"]';
		...
		await Promise.all([
			signin.waitForResponse(
				(res: any) => res.url().toLowerCase().includes('login') && [200, 204, 302].includes(res.status()),
				{ timeout: timeoutNumber }
			).catch(() => null),
		...
		const hasSession = cookies.some((c: any) => /session|token|auth|copart/i.test(c.name));
		if (!hasSession) {
		...
		const exportButtonText = await exportHandle.evaluate(
			(el: Element) => (el.textContent ?? '').trim().replace(/\s+/g, ' ')
		);
		...
		await Promise.allSettled([
			signin.waitForNavigation({ waitUntil: 'domcontentloaded', timeout: 15000 }),
			humanClickHandle(signin, exportHandle),
		]);
